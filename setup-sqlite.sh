...
# Garissa E-commerce Project Setup Script (SQLite Version)
...
echo "🚀 Starting Garissa E-commerce Project Setup (SQLite Version)..."
echo "=============================================================="
...
# Create .env file for backend (SQLite version)
print_status "Creating backend .env file (SQLite)..."
cat > .env << EOF
# Database Configuration (SQLite)
DATABASE_URI=sqlite:///garissa.db
...
# Create test users (admin, sellers, buyers)
...
"
python create_test_users.py
echo "Database reset and test users recreated"
EOF
...
cat > SETUP_COMPLETE_SQLITE.md << 'EOF'
# Garissa E-commerce Setup Complete! 🎉 (SQLite Version)
...
│   ├── garissa.db          # SQLite database file
...
## Database:
- **Database:** SQLite (garissa.db)
- **Location:** Backend/garissa.db
...
if command -v sqlite3 &> /dev/null; then
    print_success "SQLite3 $(sqlite3 --version) ✓"
else
    print_error "SQLite3 not found ✗"
fi

# Check if database file exists
if [ -f "Backend/garissa.db" ]; then
...
echo ""
echo "📋 Next Steps:"
echo "1. Read SETUP_COMPLETE_SQLITE.md for detailed information"
